...
with open(os.path.join(os.path.dirname(__file__), "interp.py"), "w") as f:
    ...
    f.write("from kirin.interp import DialectInterpreter, ResultValue, impl\n")
    f.write("\n")
    ...
    for name, obj, sig in builtin_math_functions():
        ...
        implements.append(
            f"""
    @impl(stmts.{name})
    def {name}(self, interp, stmt: stmts.{name}, values: tuple) -> ResultValue:
        return ResultValue(math.{name}({fields}))"""
        )
